function formatUsd(amountUsd) {
  return `$${Math.abs(amountUsd).toFixed(4)}`;
}
...
export default function ToolCallCard({ tool, reason, amount_usd, stripe_charge_id }) {
  return (
    <div className="mt-2 rounded-lg border border-border bg-bg/60 p-3">
      ...
      <div className="mt-2 flex flex-wrap items-center justify-between gap-2 border-t border-border pt-2 text-xs">
        <span className="font-medium text-spend">Charged {formatUsd(amount_usd)}</span>
